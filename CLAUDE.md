...
├── admin.html          # 관리자 페이지 (퀴즈 생성/배포 문제 보기/결과조회)
...
│   ├── admin-engine.js  # 관리자 페이지 로직 (배포 현황/문제 보기/결과 필터링)
│   └── generators/
...
문제 생성 (클라이언트)
  ↓
deploy(grade) → POST ────────→ "현재퀴즈" 시트 (학년별 행에 저장)
                                    ↓
                               GET (답 제거) ←──── loadQuiz(grade)
                                                     ↓
                                                 퀴즈 풀기
                                                     ↓
                               "결과" 시트 ←── submitResult()
...
**Google Sheets "현재퀴즈" 시트 구조**: Row 1=헤더, Row 2=중1, Row 3=중2, Row 4=중3 (grade N → row N+1)
...
| `localQuiz_grade{N}` | localStorage | 학년별 로컬 테스트 퀴즈 저장 |
...
| 메서드 | 액션 | 보안 | 설명 |
|---|---|---|---|
| GET | `getQuiz&grade=N` | 없음 | 학년별 퀴즈 로드 (답 제거 + fullProblems 포함) |
| GET | `getDeployedStatus&password=` | 비밀번호 | 학년별 배포 현황 조회 |
| GET | `getResults&password=` | 비밀번호 | 결과 조회 |
| POST | `deployQuiz` | 비밀번호 | 학년별 퀴즈 배포 (grade N → row N+1) |
